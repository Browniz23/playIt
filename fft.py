...
def getFFTNotes(filename):
    ...
    for i in range(iters):
        ...
        if len(curr_signal.shape) > 1:
            curr_signal = np.sum(curr_signal, axis=1)
        fft = abs(scipy.fft.fft(curr_signal))
        ...
        if maxApt < 10: # choose 10 as small enough for break
            ...
            isBreak = False
        tops = []
        # local_max = signal.argrelextrema(fft, np.greater)
        # print(local_max[0])
        for i,a in enumerate(fft):
            if a > maxApt * 2/3 and i != maxIdx: # maybe can pass middle
                n = closestPianoNoteNum(freqs[i])
                # if maxsNoteNum == 44:
                #     print("HEREEEEEEEEE", maxApt, a, n)
                ...
                if n == maxsNoteNum - 12 or n == maxsNoteNum - 24 or n == maxsNoteNum - 36 or n == maxsNoteNum - 48 or n == maxsNoteNum - 60 or n == maxsNoteNum - 72 or n == maxsNoteNum - 84:
                    print("yay?", n, maxsNoteNum)
                    maxsNoteNum = n 
# changed critical!                    # curr_main = (maxApt, maxsNoteNum) # change maxApt to a????? or to curr_main[0]???
                tops.append(n)
                # tops.append((a,i))
        ...
        if maxsNoteNum == curr_main[1]:
            print("AAAAAAAAAAAA", maxApt, curr_main[0], maxsNoteNum)
            count += 1
        elif maxApt > curr_main[0]: # and not isFading:
            print("BBBBBBBBBBBB", maxApt, curr_main[0], maxsNoteNum)
            curr_main = (maxApt, maxsNoteNum)
            count += 1
        elif curr_main[1] in tops:
            print("CCCCCCCCCCCCC", maxApt, curr_main[0], maxsNoteNum)
            # probably unprecise freq mistake
            count += 1
        else:
            print("DDDDDDDDDDDDDD", maxApt, curr_main[0], maxsNoteNum)
        ...
        isFading = False
        # else:
        #     print("EEEEEEEEEEEEEE", maxApt, curr_main[0], maxsNoteNum)
        #     isFading = True
        #     count += 1
        prevApt = maxApt
